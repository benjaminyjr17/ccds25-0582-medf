#!/usr/bin/env python3
"""
Generate the Maastricht University-tailored version of the Course Descriptions: Statistics Courses PDF.
Tailored for MSc International Business: Information Management and Business Intelligence.
Only 6 courses (dropping MH1805, MH1812, MH2802). New introduction paragraph.
"""
...
OUTPUT_PATH = "/home/ubuntu/Yick_Benjamin_Oliver_Course_Descriptions_Statistics_Courses_Maastricht.pdf"

# Colors (exact from original PDF)
TITLE_COLOR = (68, 114, 195)
...
class CourseDescriptionPDF(FPDF):
    def __init__(self):
        ...
        # Add Carlito fonts
        self.add_font("Carlito", "", os.path.join(FONT_DIR, "Carlito-Regular.ttf"))
        self.add_font("Carlito", "B", os.path.join(FONT_DIR, "Carlito-Bold.ttf"))
        self.add_font("Carlito", "I", os.path.join(FONT_DIR, "Carlito-Italic.ttf"))
        self.add_font("Carlito", "BI", os.path.join(FONT_DIR, "Carlito-BoldItalic.ttf"))
...
def build_pdf():
    ...
    pdf.cell(0, 10, "Course Descriptions: Statistics Courses", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(3)

    # Introduction Paragraph (Maastricht-tailored)
    pdf.set_text_color(*BLACK)
    pdf.set_font("Carlito", "", 10)

    intro_text = (
        f"This document provides detailed descriptions for all relevant statistics courses "
        f"completed by **Benjamin Oliver Yick** during his Bachelor of Computing (Hons) in Data Science and "
        f"Artificial Intelligence at **Nanyang Technological University, Singapore (NTU)**, an institution "
        f"whose Nanyang Business School holds both AACSB and EQUIS accreditation. It is submitted to the "
        f"Board of Admission at the **School of Business and Economics, Maastricht University**, as direct "
        f"evidence of the required statistical and quantitative background for admission to the **Master of "
        f"International Business**, with a specialization in **Information Management and Business Intelligence**. "
        f"As requested, this document lists the name of each course, the number of credits received, and a "
        f"short course description (each within 10 sentences). The six courses detailed herein total 42 ECTS "
        f"and encompass probability theory, mathematical statistics, applied data analysis, sampling and survey "
        f"methodology, information analytics, and the application of quantitative models for business "
        f"decision-making. These courses, drawn from both the School of Physical and Mathematical Sciences and "
        f"the Nanyang Business School, collectively demonstrate the level of statistics and mathematics "
        f"evaluated by the Board of Admission and confirm that the applicant's undergraduate program corresponds "
        f"with the master's program in its emphasis on data management, statistical modeling, and "
    ...
    pdf.section_title("Overview of statistics courses")
    ...
    pdf.add_course(
        ...
        f"Optimization models for business decisions; Simulation; Inventory control; Waiting line management (queueing theory); Forecasting; Decision making under uncertainty.",
    ...
    pdf.add_course(
        ...
        f"Review of probability; Random samples, sample mean and sample variance; Distributions derived from the normal distribution; Central Limit Theorem and its significance for statistics; Introduction to parameter estimation and quality criteria for estimators; Constructing good estimators: method of moments and maximum likelihood method; Asymptotic properties of estimators, Cram\u00e9r-Rao bound and efficient estimators; Confidence intervals for estimators; Introduction to hypothesis testing and Fisher-type tests; Neyman-Pearson tests and Neyman-Pearson Lemma.",
