...
\end{tikzpicture}
\caption{Scoring Aggregation: multi-framework evaluation pipeline.}
...
The \textbf{evaluation router} classifies the TOPSIS closeness coefficient $C$ (where higher values indicate better ethical performance) using the thresholds in Table~\ref{tab:risk_levels_eval}.

\begin{table}[H]
\centering
\caption{Risk Classification: evaluation thresholds based on closeness coefficient $C$.}
\label{tab:risk_levels_eval}
\begin{tabular}{@{}lll@{}}
\toprule
...
The \textbf{harm assessment module} classifies a composite harm score $h$ (where higher values indicate greater harm) using the thresholds in Table~\ref{tab:risk_levels_harm}. The harm score is computed as $h = 0.7 \times (1 - s_{\text{norm}}) + 0.3 \times \bar{d}$, where $s_{\text{norm}}$ is the normalized dimension score and $\bar{d}$ is the mean pairwise stakeholder weight disagreement. The 0.7/0.3 weighting reflects a deliberate design decision: a system's own ethical performance should dominate its risk classification, with stakeholder disagreement serving as a secondary aggravating factor that elevates the harm estimate when governance consensus is lacking.

\begin{table}[H]
\centering
\caption{Harm Severity Classification: thresholds based on harm score $h$.}
\label{tab:risk_levels_harm}
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Harm Score Range} & \textbf{Severity} & \textbf{Description} \\
\midrule
...
These two systems are conceptually consistent: the closeness coefficient is a \enquote{higher is better} metric while the harm score is a \enquote{higher is worse} metric. A system with a low closeness coefficient will tend to have a high harm score, and vice versa. The harm assessment also considers the level of stakeholder conflict. A system with a moderate overall score but high stakeholder conflict may be flagged as higher risk than its score alone would suggest, reflecting the additional governance challenge posed by unresolved ethical disagreements.
...
\begin{tikzpicture}[
  ...
  \draw[arrow] (harm) -- (out);
\end{tikzpicture}
\caption{End-to-End Evaluation Pipeline: scoring, aggregation, and harm assessment stages.}
\label{fig:pipeline}
\end{figure}
