...
\begin{tikzpicture}[
  ...
  comp/.style={rectangle, draw, fill=white, text width=7em, text centered, minimum height=2em, font=\small, rounded corners=2pt},
...
\end{tikzpicture}
\caption{System Architecture: three-tier platform design.}
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{Conflict Analysis} (\texttt{routers/conflicts.py}): Implements the full stakeholder conflict analysis workflow, including Spearman's rank correlation, divergent-dimension identification, conflict severity determination, and integration with the harm-assessment module. In the current codebase, this logic remains within the router layer rather than a dedicated engine module, which is a maintainability limitation rather than a missing functional capability.
...
\begin{tikzpicture}[
  ...
  mod/.style={rectangle, draw, fill=blue!8, text width=10.5em, text centered, minimum height=2em, font=\small, rounded corners=2pt},
  ...
  \node[mod, below=1cm of conf_r] (harm) {\texttt{harm\_assessment.py}};
...
\end{tikzpicture}
\caption{Software Module Diagram: component dependencies and data flow.}
\label{fig:modules}
\end{figure}
