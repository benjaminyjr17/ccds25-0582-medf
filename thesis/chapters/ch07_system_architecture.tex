...
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{Conflict Detection} (\texttt{conflict\_detection.py}): Contains the logic for comparing stakeholder weightings and calculating conflict metrics like Spearman's rank correlation.
  \item \textbf{Harm Assessment} (\texttt{harm\_assessment.py}): Implements a model to estimate the potential severity of ethical risks based on system scores and stakeholder disagreement.
  \item \textbf{Pareto Engine} (\texttt{routers/pareto.py}): Integrates the NSGA-II algorithm to compute the Pareto frontier of consensus weightings.
\end{itemize}
...
\begin{tikzpicture}[
  ...
  \node[mod, below=1cm of conf_r] (conflict) {\texttt{conflict\_detection.py}};
