...
\begin{table}[H]
\centering
\caption{Default Stakeholder Weight Profiles: developer, regulator, and affected community priorities.}
\label{tab:weights}
\begin{tabular}{@{}lccc@{}}
\toprule
...
\begin{table}[H]
\centering
\caption{Baseline Dimension Scores: live facial recognition system inputs.}
...
\begin{table}[H]
\centering
\caption{Evaluation Results: live facial recognition per-stakeholder scores.}
...
\begin{table}[H]
\centering
\caption{Stakeholder Conflict Matrix: live facial recognition contribution-based Spearman $\rho$.}
...
\begin{table}[H]
\centering
\caption{Baseline Dimension Scores: iTutorGroup hiring algorithm system inputs.}
...
\begin{table}[H]
\centering
\caption{Evaluation Results: iTutorGroup hiring algorithm per-stakeholder scores.}
...
The overall aggregated score was approximately 0.35, corresponding to a \textbf{Critical Risk} classification. This case study demonstrates MEDF's ability to correctly identify and penalize systems with a single, catastrophic ethical flaw. Even though the system had a moderate Accountability score (reflecting that the company was eventually held accountable), the failure on Fairness was so severe that it rendered the complete system unacceptable from every stakeholder's perspective.
...
\begin{table}[H]
\centering
\caption{Stakeholder Conflict Matrix: iTutorGroup contribution-based Spearman $\rho$.}
...
\begin{table}[H]
\centering
\caption{Baseline Dimension Scores: NHS-DeepMind Streams system inputs.}
...
\begin{table}[H]
\centering
\caption{Evaluation Results: NHS-DeepMind Streams per-stakeholder scores.}
...
\begin{table}[H]
\centering
\caption{Stakeholder Conflict Matrix: NHS-DeepMind Streams contribution-based Spearman $\rho$.}
...
\begin{table}[H]
\centering
\caption{Cross-Case Comparison: evaluation results across all three case studies.}
\label{tab:cross_case}
\small
\begin{tabular}{@{}lccl>{\centering\arraybackslash}p{2cm}c@{}}
\toprule
\textbf{Case Study} & \textbf{Overall} & \textbf{Risk} & \textbf{Weakness} & \textbf{Min Pairwise $\rho$} & \textbf{Conflict} \\
\midrule
