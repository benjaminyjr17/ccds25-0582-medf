...
The \texttt{app/routers/} directory contains the individual FastAPI routers for each major API endpoint. For example, \texttt{evaluate.py} handles the logic for the \texttt{/api/evaluate} endpoint. It receives the \texttt{EvaluateRequest} payload, coordinates calls to the scoring engine and framework registry, aggregates the results, and returns a validated \texttt{EvaluationResult} response. This modular approach keeps the endpoint logic clean and organized.
