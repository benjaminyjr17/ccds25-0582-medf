...
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{Incomplete Separation of Concerns.} The conflict analysis logic is currently implemented inline within the API router (\texttt{routers/conflicts.py}) rather than in the dedicated engine module (\texttt{conflict\_detection.py}), which remains a placeholder stub. While the functionality is complete and all tests pass, this architectural shortcut represents technical debt. A future refactoring should extract the conflict analysis logic into the standalone module to complete the intended separation of concerns.
\end{itemize}
